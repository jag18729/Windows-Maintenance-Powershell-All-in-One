...
function Show-Header {
    Clear-Host
    Write-Host ""
    Write-Host "Windows System Maintenance Utility" -ForegroundColor Green
...
# Main menu function with arrow key navigation
function Show-MainMenu {
    Show-Header
    
    Write-Host "Select operation using arrow keys and Enter:" -ForegroundColor White
    Write-Host ""
    
    $menuItems = @(
        @{ Description = "Install Windows Updates"; Function = { Update-Windows } },
        @{ Description = "Scan and Repair System Files"; Function = { Repair-SystemFiles } },
        @{ Description = "Check and Fix Disk Errors"; Function = { Repair-DiskErrors } },
        @{ Description = "Optimize Drives"; Function = { Optimize-Drives } },
        @{ Description = "Clean Temporary Files"; Function = { Remove-TempFiles } },
        @{ Description = "Optimize Network"; Function = { Optimize-Network } },
        @{ Description = "Run Memory Diagnostic"; Function = { Start-MemoryDiagnostic } },
        @{ Description = "Optimize Power Settings"; Function = { Optimize-PowerSettings } },
        @{ Description = "Run Full System Maintenance"; Function = { Start-FullMaintenance } },
        @{ Description = "Exit Program"; Function = { return $false } }
    )
    ...
    function Display-Menu {
        ...
        for ($i = 0; $i -lt $menuItems.Count; $i++) {
            if ($i -eq $selectedIndex) {
                ...
                Write-Host "$($i + 1). $($menuItems[$i].Description)" -ForegroundColor Green
            } else {
                Write-Host "   $($i + 1). $($menuItems[$i].Description)" -ForegroundColor Cyan
            }
        }
        Write-Host ""
        Write-Host "Use ↑/↓ arrows to select, Enter to confirm, Q to quit" -ForegroundColor DarkGray
    }
    ...
    while ($true) {
        ...
        switch ($key.VirtualKeyCode) {
            ...
            13 { # Enter
                Clear-Host
                if ($selectedIndex -eq 9) { # Exit option
                    return $false
                }
                & $menuItems[$selectedIndex].Function
                return $true
            }
            81 { # Q key
                return $false
            }
            57 { # 9 key - direct selection for "Run Full System Maintenance"
                if ($key.Character -eq '9') {
                    Clear-Host
                    Start-FullMaintenance
                    return $true
                }
            }
            { $_ -ge 49 -and $_ -le 56 } { # Number keys 1-8
                $num = $key.Character.ToString()
                if ($num -match '^\d$') {
                    $index = [int]$num - 1
...
while ($continue) {
    ...
    if ($continue) {
        Write-Host ""
        Write-MatrixText "Press any key to return to the command interface..." "Yellow"
